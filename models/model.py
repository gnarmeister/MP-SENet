...
class DenseEncoder(nn.Module):
    def __init__(self, h, in_channel):
        ...
        self.dense_conv_1 = nn.Sequential(
            nn.Conv2d(in_channel, h.dense_channel, (1, 1)), nn.InstanceNorm2d(h.dense_channel, affine=True), nn.PReLU(h.dense_channel)
        )
...
class PhaseDecoder(nn.Module):
    def __init__(self, h, out_channel=1):
        ...
        self.phase_conv = nn.Sequential(
            SPConvTranspose2d(h.dense_channel, h.dense_channel, (1, 3), 2), nn.InstanceNorm2d(h.dense_channel, affine=True), nn.PReLU(h.dense_channel)
        )
...
class MPNet(nn.Module):
    ...
    def forward(self, noisy_amp, noisy_pha, noise_log_extended):  # [B, F, T]

        x = torch.stack((noisy_amp, noisy_pha, noise_log_extended), dim=-1).permute(0, 3, 2, 1)  # [B, 3, T, F]
        ...
        denoised_com = torch.stack((denoised_amp * torch.cos(denoised_pha), denoised_amp * torch.sin(denoised_pha)), dim=-1)
...
def pesq_score(utts_r, utts_g, h):
    ...
    pesq_score = Parallel(n_jobs=30)(
        delayed(eval_pesq)(utts_r[i].squeeze().cpu().numpy(), utts_g[i].squeeze().cpu().numpy(), h.sampling_rate) for i in range(len(utts_r))
    )
